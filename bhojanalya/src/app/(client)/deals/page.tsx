...
export default function DealsPage() {
  ...
  const handleRemoveActiveDeal = async (dealId: string) => {
    try {
      ...
      showToast("Could not remove deal", "error");
    }
  };
                ...
                {onboardingStatus === "BOTH_COMPLETED" ? (
                  <>
                    ...
                    <div className="flex-1 overflow-y-auto pr-1 space-y-3 pb-6">
                        {suggestedDeals.map((deal) => (
                            <div key={deal.id} onClick={() => handleAddSuggestion(deal)} className="p-4 bg-white border border-slate-100 rounded-2xl hover:border-[#f1cd48] hover:shadow-md cursor-pointer transition-all group">
                                ...
                                <div className="flex gap-2 mt-3">
                                    ...
                                    <span className="text-[9px] bg-[#f1cd48]/10 px-2 py-1 rounded-md text-amber-700 font-bold uppercase">
                  ...
                  <>
                    <div className="flex items-center gap-2 mb-4 flex-shrink-0">
                        ...
                        <h3 className="font-bold text-sm uppercase tracking-wide text-slate-700">Active Deals</h3>
                    </div>
                    <div className="flex-1 overflow-y-auto pr-1 space-y-3 pb-6">
                        {activeDeals.length === 0 && <p className="text-[10px] text-slate-400 text-center mt-10">No active deals found.</p>}
                        {activeDeals.map((deal) => (
                            <div key={deal.id} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl relative group">
                                <button onClick={() => handleRemoveActiveDeal(deal.id)} className="absolute top-3 right-3 text-slate-300 hover:text-red-500 transition-colors">
                                  <Trash2 size={14} />
                                </button>
                                <span className="text-xs font-bold text-slate-700 block mb-2">{deal.title}</span>
                                <div className="flex gap-2">
                                    <span className="text-[9px] bg-white px-2 py-1 rounded-md border border-slate-200 text-slate-500 font-bold uppercase">{deal.category}</span>
                                    <span className="text-[9px] bg-green-500/10 px-2 py-1 rounded-md text-green-700 font-bold uppercase">
                                        {deal.type === 'percentage' ? `${deal.discount_value}% OFF` : `₹${deal.discount_value} FLAT`}
                                    </span>
                                </div>
                            </div>
          ...
          <div className="lg:col-span-2 flex flex-col h-full min-h-0">
             <div className="bg-white border border-slate-200 rounded-[2rem] p-8 flex flex-col h-full shadow-sm relative overflow-hidden">
                <div className="flex-shrink-0 mb-4 pb-4 border-b border-slate-100">
                    ...
                    <div className="grid grid-cols-12 gap-4">
                        <div className="col-span-8 space-y-1.5">
                            ...
                            <input type="text" value={formTitle} onChange={(e) => setFormTitle(e.target.value)} className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl px-4 text-sm font-bold focus:outline-none focus:ring-2 ring-[#471396]/5" placeholder="e.g. Weekend Special" />
                        </div>
                    ...
                    <div className="flex items-end gap-4 mt-4">
                         ...
                         <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-200 h-12">
                            <button onClick={() => setFormType('percentage')} className={`px-4 rounded-lg text-xs font-bold uppercase transition-all ${formType === 'percentage' ? 'bg-[#471396] text-white shadow-sm' : 'text-slate-400'}`}>%</button>
                            <button onClick={() => setFormType('flat')} className={`px-4 rounded-lg text-xs font-bold uppercase transition-all ${formType === 'flat' ? 'bg-[#471396] text-white shadow-sm' : 'text-slate-400'}`}>₹</button>
                        </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pb-24 scroll-hide">
                    {draftDeals.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-slate-300 p-8 grayscale opacity-30">
                          <Megaphone size={32} className="mb-2" />
                          <span className="text-xs font-medium">No deals in draft</span>
                        </div>
                    ) : (
                        draftDeals.map((deal) => (
                            <div key={deal.id} className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-[#471396] transition-all">
                                <div>
                ...
                <div className="absolute bottom-0 left-0 w-full p-8 bg-white z-20 border-t border-slate-50">
